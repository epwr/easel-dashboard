...
class GraphicDashboard {
  ...
  load() {
    ...
    if (!this.initialized){
      this.elementInfo.forEach((elemInfo, index) => { // TODO: test removing index
        ...
        let elem = createElement(elemInfo['type'], elemInfo['name'], elemInfo['desc'], elemInfo['dataTypes']);
        wrapper.appendChild(elem.node);
  ...
  addData(elem_id, dataType, label, value) {
    ...
    if (this.initialized) {
      ...
      console.log("I need to store data somewhere");
    }
  }
}
