...
const createElement = (type, name, desc, dataTypes, size) => {
  switch(type) {
  case 'time-series':
    return new TimeSeriesElement(name, desc, dataTypes, size);
  default:
...
class TimeSeriesElement {
   ...
   *
   */
  constructor(name, desc, dataTypes, size) {
    ...
    if (size > 1){
      this.node.style.gridColumnStart = "1";
      this.node.style.gridColumnEnd = "3";
      console.log("Hererererer");
    }
    if (size > 2){
      this.node.style.gridRowStart = "1";
      this.node.style.gridRowEnd = "3";
    }
